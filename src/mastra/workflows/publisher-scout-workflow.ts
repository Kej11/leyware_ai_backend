...
function transformToScoutResults(analysis: any, scout: any) {
  const results: any[] = [];
  ...
  // Transform watch list items with moderate relevance
  analysis.watchList.forEach((item: any) => {
    results.push({
      title: item.title,
      source_url: item.url,
      author: item.developer,
      author_url: null,
      content: `Watch for: ${item.whatToWatch}`,
      relevance_score: 0.5,
      ...
      platform: 'itch.io',
      analysis_reasoning: item.whatToWatch || 'Monitor for future potential'
    });
  });
...
const storeResultsStep = createStep({
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    logger.info('💾 Transforming and storing results in database...', {
      scoutId: scout.id,
      runId: run_id,
    ...
    try {
      // Transform all results to database format
      const dbResults = transformToScoutResults(analysis, scout);
      
      if (dbResults.length > 0) {
        // Use batch storage for all results
...
const generateReportStep = createStep({
  ...
  execute: async ({ inputData, mastra }) => {
    ...
    logger.info(`   Immediate Opportunities: ${report.statistics.immediateActions}`);
    logger.info(`   High Priority: ${report.statistics.highPriority}`);
    logger.info(`   Watch List: ${report.statistics.watchList}`);
...
// Create the workflow
export const publisherScoutWorkflow = createWorkflow({
  id: 'publisher-scout-workflow',
  description: 'Scout itch.io for game investment opportunities using scout configuration',
